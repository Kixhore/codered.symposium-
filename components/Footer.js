          ...
          <div className="pt-8 border-t border-[var(--glass-border)] flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="flex gap-4 mt-4 md:mt-0">
              {['twitter', 'instagram', 'github'].map((social) => (
                <button key={social} className="w-8 h-8 rounded-full bg-white bg-opacity-10 flex items-center justify-center hover:bg-white hover:bg-opacity-20 transition-all duration-300">
                  <div className={`icon-${social} text-sm text-[var(--neon-red)]`}></div>
                </button>
              ))}
              <a 
